...
function setOptions(options) {
  if (options) {
      ...
      slides.forEach(
        slide =>
          (slide.style.animationDuration =
            options.animationDuration / 1000 + 's')
      );

      document.querySelectorAll(
        '.make-slides section .appear li'
      ).style.transition = `opacity ${options.animationDuration /
        1000}s linear`;
    }
    ...
    if (options.transition === null) {
      ...
      const appears = document.querySelectorAll(
        '.make-slides section .appears li'
      );
      if (appears.length > 0) {
